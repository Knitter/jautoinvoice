...
<?php $form=$this->beginWidget('CCodeForm', array('model'=>$form)); ?>
	...
	<div class="row sticky">
		<?php echo $form->labelEx($form, 'connectionId')?>
		<?php echo $form->textField($form, 'connectionId', array('size'=>65))?>
		<div class="tooltip">
		The database component that should be used.
		</div>
		<?php echo $form->error($form,'connectionId'); ?>
	</div>
	<div class="row sticky">
		<?php echo $form->labelEx($form,'tablePrefix'); ?>
		<?php echo $form->textField($form,'tablePrefix', array('size'=>65)); ?>
		<div class="tooltip">
		...
		<?php echo $form->error($form,'tablePrefix'); ?>
	</div>
	<div class="row">
		<?php echo $form->labelEx($form,'tableName'); ?>
		<?php $this->widget('zii.widgets.jui.CJuiAutoComplete',array(
			'model'=>$form,
			'attribute'=>'tableName',
			'name'=>'tableName',
			'source'=>Yii::app()->hasComponent($form->connectionId) ? array_keys(Yii::app()->{$form->connectionId}->schema->getTables()) : array(),
			'options'=>array(
				'minLength'=>'0',
				'focus'=>new CJavaScriptExpression('function(event,ui) {
					$("#'.CHtml::activeId($form,'tableName').'").val(ui.item.label).change();
					return false;
				}')
			),
			'htmlOptions'=>array(
				'id'=>CHtml::activeId($form,'tableName'),
				'size'=>'65',
		...
		<?php echo $form->error($form,'tableName'); ?>
	</div>
	<div class="row model-class">
		<?php echo $form->label($form,'modelClass',array('required'=>true)); ?>
		<?php echo $form->textField($form,'modelClass', array('size'=>65)); ?>
		<div class="tooltip">
		This is the name of the model class to be generated (e.g. <code>Post</code>, <code>Comment</code>).
		It is case-sensitive.
		</div>
		<?php echo $form->error($form,'modelClass'); ?>
	</div>
	<div class="row sticky">
		<?php echo $form->labelEx($form,'baseClass'); ?>
		<?php echo $form->textField($form,'baseClass',array('size'=>65)); ?>
		<div class="tooltip">
			This is the class that the new model class will extend from.
			Please make sure the class exists and can be autoloaded.
		</div>
		<?php echo $form->error($form,'baseClass'); ?>
	</div>
	<div class="row sticky">
		<?php echo $form->labelEx($form,'modelPath'); ?>
		<?php echo $form->textField($form,'modelPath', array('size'=>65)); ?>
		<div class="tooltip">
		...
		<?php echo $form->error($form,'modelPath'); ?>
	</div>
	<div class="row">
		<?php echo $form->labelEx($form,'buildRelations'); ?>
		<?php echo $form->checkBox($form,'buildRelations'); ?>
		<div class="tooltip">
		...
		<?php echo $form->error($form,'buildRelations'); ?>
	</div>
	<div class="row">
		<?php echo $form->labelEx($form,'commentsAsLabels'); ?>
		<?php echo $form->checkBox($form,'commentsAsLabels'); ?>
		<div class="tooltip">
		...
		<?php echo $form->error($form,'commentsAsLabels'); ?>
	</div>
