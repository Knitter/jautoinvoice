<div class="row template sticky">
	<?php echo $this->labelEx($form,'template'); ?>
	<?php echo $this->dropDownList($form,'template',$templates); ?>
	<div class="tooltip">
		Please select which set of the templates should be used to generated the code.
	</div>
	<?php echo $this->error($form,'template'); ?>
</div>

<div class="buttons">
	<?php echo CHtml::submitButton('Preview',array('name'=>'preview')); ?>

	<?php if($form->status===CCodeModel::STATUS_PREVIEW && !$form->hasErrors()): ?>
...
<?php if(!$form->hasErrors()): ?>
	<div class="feedback">
	<?php if($form->status===CCodeModel::STATUS_SUCCESS): ?>
		<div class="success">
			<?php echo $form->successMessage(); ?>
		</div>
	<?php elseif($form->status===CCodeModel::STATUS_ERROR): ?>
		<div class="error">
			<?php echo $form->errorMessage(); ?>
		</div>
	<?php endif; ?>

	<?php if(isset($_POST['generate'])): ?>
		<pre class="results"><?php echo $form->renderResults(); ?></pre>
	<?php elseif(isset($_POST['preview'])): ?>
		...
		<table class="preview">
			...
			<?php foreach($form->files as $i=>$file): ?>
				...
				<td class="confirm">
					...
					else
					{
						...
						echo CHtml::label($file->operation, "answers_{$key}")
							. ' ' . CHtml::checkBox("answers[$key]", $form->confirmed($file));
					}
					?>
				</td>
			</tr>
